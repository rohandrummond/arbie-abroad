...
function CreatePost() {
  ...
  function handleFileChange(e) {
    ...
    previewFile(file, id)
    setFiles((prevFiles) => {
      let duplicateSearch = fileTracker.filter(
        (trackedFile) => trackedFile.fileId === id
      )
      if (duplicateSearch.length !== 0) {
        let deduplicatedFiles = prevFiles.filter(
          (prevFile) => prevFile.name !== duplicateSearch[0].fileName
        )
    ...
    setFileTracker((prevFileTracker) => {
      let deduplicatedFileTracker = prevFileTracker.filter(
        (trackedFile) => trackedFile.fileId !== id
      )
      return [
        ...deduplicatedFileTracker,
        {
          fileId: id,
          fileName: file.name,
        },
        ...
        <div className="post-form-ctr">
          ...
          <form encType="multipart/form-data" onSubmit={handlePost}>
            ...
            <div className="flex row form-inpt-grp form-img-upld-ctr">
              <label
                htmlFor="first-image"
                className="flex column form-img-upld-lbl"
              >
                <span className="body-txt form-inpt-labl">First Image</span>
                <span className="btn-outline">Choose</span>
                {fileTracker.find(
                  (trackedFile) => trackedFile.fileId === 'first-image'
                ) && (
                  <span className="sub-txt form-img-filename ">
                    {
                      fileTracker.find(
                        (trackedFile) => trackedFile.fileId === 'first-image'
                      ).fileName
                ...
                {fileErrors['first-image'] ? (
                  <p className="sub-txt form-err">
            ...
            <div className="flex row form-inpt-grp form-img-upld-ctr">
              ...
              >
                ...
                <span className="btn-outline">Choose</span>
                {fileTracker.find(
                  (trackedFile) => trackedFile.fileId === 'second-image'
                ) && (
                  <span className="sub-txt form-img-filename ">
                    {
                      fileTracker.find(
            ...
            <div className="flex column form-inpt-grp">
              <label className="body-txt form-inpt-labl">
                Second Paragraph
              </label>
              <textarea
                id="second-paragraph"
